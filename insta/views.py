...
def get_posts(request):
    ...
    sql = """
 select u.username, p.* , f.*, (l.id is not null) as isLiked, c.comment_body
 FROM insta_post p 
 LEFT JOIN insta_follow f ON (p.user_id = f.user_id )
 LEFT JOIN auth_user u ON (u.id = p.user_id)
 LEFT JOIN insta_comment c ON (c.post_id = p.id AND c.user_id = """+str(request.user.id)+""")
 LEFT JOIN insta_likes l ON (l.post_id = p.id AND l.user_id = """+str(request.user.id)+""")
 WHERE p.user_id = """+str(request.user.id)+""" OR f.user_id is NOT Null
 ORDER BY p.created_at DESC
    ...
    for postval in page_obj:
        post_id_val = postval[1]
        liked = get_like_status(request,post_id_val)
        if(liked):
            liked_val = True

        liked_val = postval[14]
        post_user = postval[0]
        posts[i] = {'id': post_id_val,'image': postval[2],'caption': postval[3],'liked':liked_val,'post_user' : post_user,'post_comment':postval[15],}
        i += 1
    return posts
...
def likeme(request):
    ...
    return JsonResponse({"data":"success","id":post_id,"unlike":unlike})
...
def comment(request):
    ...
    return JsonResponse({"data":"success",'comment':comment_val})
